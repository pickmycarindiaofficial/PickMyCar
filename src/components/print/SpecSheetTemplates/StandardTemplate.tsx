...
export default function StandardTemplate({ carData, dealerProfile, selectedFields }: StandardTemplateProps) {
  ...
  return (
    <div className="spec-sheet-container" style={{ width: '210mm', height: '297mm', padding: '10mm', background: 'white', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
      {/* Header */}
      <div className="spec-sheet-header" style={{ borderBottom: '3px solid #000', paddingBottom: '5px', marginBottom: '10px' }}>
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            <div className="flex items-center gap-2 justify-end text-2xl font-extrabold">
      ...
      <div className="spec-sheet-body" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <table className="spec-table" style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '10px' }}>
          <tbody>
            {hasField('brand_model') && (
              <tr>
                <td className="label" style={{ border: '1px solid #333', padding: '6px 12px', fontWeight: 'bold', backgroundColor: '#f5f5f5', width: '35%', fontSize: '18px' }}>
                  Brand & Model
                </td>
                <td className="value" style={{ border: '1px solid #333', padding: '6px 12px', fontSize: '24px', fontWeight: '800' }}>
            ...
            {hasField('variant') && carData?.variant && (
              <tr>
                <td className="label" style={{ border: '1px solid #333', padding: '6px 12px', fontWeight: 'bold', backgroundColor: '#f5f5f5', fontSize: '18px' }}>
                  Variant
                </td>
                <td className="value" style={{ border: '1px solid #333', padding: '6px 12px', fontSize: '20px', fontWeight: '600' }}>
                  {carData.variant}
                </td>
              </tr>
            )}
            ...
            {hasField('year') && (
              <tr>
                ...
                <td className="value" style={{ border: '1px solid #333', padding: '6px 12px', fontSize: '20px' }}>
            ...
            {hasField('registration_number') && carData?.registration_number && (
              <tr>
                ...
                <td className="value" style={{ border: '1px solid #333', padding: '6px 12px', fontSize: '20px', textTransform: 'uppercase' }}>
            ...
            {hasField('insurance_validity') && carData?.insurance_status === 'expired' && (
              <tr>
                ...
                <td className="value" style={{ border: '1px solid #333', padding: '6px 12px', fontSize: '20px', color: '#ea580c', fontWeight: '600' }}>
                  Expired
                </td>
              </tr>
            )}
        ...
        {hasField('features') && features.length > 0 && (
          <div style={{ marginTop: '5px', flex: 1, overflow: 'hidden' }}>
            <h3 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '8px', borderBottom: '2px solid #000', paddingBottom: '2px' }}>
              Features
            </h3>
            <div className="features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px 12px', alignContent: 'start' }}>
              {features.slice(0, 10).map((feature: string, index: number) => (
                <div key={index} className="feature-item" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '18px', fontWeight: '500', lineHeight: 1.2 }}>{feature}</span>
                </div>
              ))}
            </div>
          </div>
