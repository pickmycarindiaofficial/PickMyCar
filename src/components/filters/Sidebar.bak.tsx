...
export const Sidebar = ({ filters, onFilterChange, onClearAll, isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 right-0 z-50 h-full w-full bg-white transition-transform duration-300 lg:sticky lg:top-20 lg:z-0 lg:h-[calc(100vh-5rem)] lg:w-72 lg:translate-x-0 border-r border-gray-100 ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="flex h-full flex-col shadow-sm">
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-100 hidden lg:flex">
            <h2 className="text-base font-bold text-gray-800 uppercase tracking-tight">Filters</h2>
            <Button variant="ghost" size="sm" onClick={onClearAll} className="text-xs text-blue-600 hover:text-blue-700 p-0 h-auto">
              Reset All
            </Button>
          </div>
            ...
            <FilterSection title="Location">
              <div className="relative">
                <select
                  ...
                  className="w-full rounded-md border border-gray-200 bg-white text-gray-700 px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 appearance-none transition-all"
                >
                  <option value="All Cities">Search City...</option>
