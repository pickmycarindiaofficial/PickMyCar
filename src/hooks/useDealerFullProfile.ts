...
export function useDealerFullProfile(dealerId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        // Fetch profile data - with graceful handling for RLS issues
        ...
        // Log errors for monitoring but continue (profiles might be blocked by RLS)
        if (profileError && profileError.code !== 'PGRST116' && profileError.code !== '42501') {
          console.warn('[useDealerFullProfile] Profile fetch error:', profileError);
        }

      // Fetch dealer profile data with visibility flags
      ...
      // Require at least dealer_profiles data (profile is optional due to RLS)
      if (!dealerData) {
      ...
      // Construct the full profile with fallbacks (profile data may be null due to RLS)
      const result: DealerFullProfile = {
        id: dealerId,
